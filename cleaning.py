...
def standardize_lgu(name):
    ...
    name = re.sub(r'\s+(PROVINCE|CITY)$', '', name)
    return name
...
# Load fiscal data
fiscal_df = pd.read_excel('fiscal_data.xlsx')
fiscal_df['LGU_clean'] = fiscal_df['LGU name'].apply(standardize_lgu)

# Rename columns (keep all spending categories)
fiscal_df.rename(columns={
    ...
    'Social Services Expenditures': 'socserv_mn',        # broader social spending
    ...
    'government expenditures in millions': 'gov_mn',     # general admin
...
spending_cols = ['health_mn', 'educ_mn', 'pubwelf_mn', 'socserv_mn', 'econdev_mn',
                 'labor_mn', 'housing_mn', 'gov_mn', 'totexp_mn']
spending_arrays = {col: fiscal_df[col].values for col in spending_cols if col in fiscal_df.columns}

# Post-election averages (t+1 to t+3)
for col in spending_arrays:
    post_vals = []
    for row in merged.itertuples():
        ...
        if np.any(mask):
            post_vals.append(np.nanmean(spending_arrays[col][mask]))
        else:
            post_vals.append(np.nan)
    merged[f'{col}_post3'] = post_vals

# Pre-election averages (t-3 to t-1) – optional, but good for control
for col in spending_arrays:
    pre_vals = []
    for row in merged.itertuples():
        lgu = row.LGU_clean
        ...
        if np.any(mask):
            pre_vals.append(np.nanmean(spending_arrays[col][mask]))
        else:
...
# Rename local revenue column
if 'local_rev_mn' in merged.columns:
    merged.rename(columns={'local_rev_mn': 'local_rev_pc'}, inplace=True)

# Select output columns (all post spending + confounders + dynasty)
output_cols = ['LGU_clean', 'election_year', 'incumbent_name', 'dynasty',
               'ira_share', 'local_rev_pc', 'enc_gol', 'income_class', 'region']
# Add all post spending columns
for col in spending_cols:
    if f'{col}_post3' in merged.columns:
        output_cols.append(f'{col}_post3')
# Optional: add pre spending as controls (if you want for matching)
for col in spending_cols:
    if f'{col}_pre3' in merged.columns:
        output_cols.append(f'{col}_pre3')

final_df = merged[output_cols].copy()
final_df.to_csv('full_panel_all_sectors.csv', index=False)
print(f"Saved full_panel_all_sectors.csv with {len(final_df)} rows and {len(output_cols)} columns.")